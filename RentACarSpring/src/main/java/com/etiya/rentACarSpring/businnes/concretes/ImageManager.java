...
public class ImageManager implements ImageService {
	...
	public Result Add(CreateImageRequest createImageRequest) throws IOException {
		...
		var result = BusinnessRules.run(checkCarImagesCount(createImageRequest.getCarId(), 5),
				...
				checkCarIsExists(createImageRequest.getCarId())
		);
	...
	public Result Update(UpdateImageRequest updateImageRequest) throws IOException {
		...
		var result = BusinnessRules.run(checkCarImagesCount(image.getCar().getCarId(), 5),
		        ...
		        checkImageIsExists(updateImageRequest.getImageId())
		);
	...
	public Result Delete(DeleteImageRequest deleteImageRequest) {
		...
		Result result = BusinnessRules.run( checkImageIsExists(deleteImageRequest.getImageId()) );
		if (result != null) {
			return result;
		}
    ...
    // Araba Kontrolü
	private Result checkCarIsExists (int CarId) {
		if ( !this.carService.checkCarExists(CarId).isSuccess() ) { // Id farkı ne ?
			return new ErrorResult("Böyle bir araba yok");
		}
		return new SuccesResult();
	}
	//Resim kontrolü
	private Result checkImageIsExists (int imageId) {
